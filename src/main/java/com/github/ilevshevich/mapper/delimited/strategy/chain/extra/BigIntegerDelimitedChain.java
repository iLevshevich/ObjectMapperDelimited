...
import com.github.ilevshevich.mapper.delimited.annotation.extra.BigIntegerDelimitedField;
...
public class BigIntegerDelimitedChain<T, U extends DelimetedModel> implements DelimitedChain<T, U> {
    private DelimitedChain<T, U> next = null;

    public BigIntegerDelimitedChain(final DelimitedChain<T, U> next) {
        this.next = next;
    }

    public BigIntegerDelimitedChain() {
        this.next = null;
    }

    @Override
    public void execute(final Annotation annotation, final Field field, final List<T> data, final U tuple) throws Exception {
        if (annotation instanceof BigIntegerDelimitedField) {
            final BigIntegerDelimitedField fieldAnnotation = (BigIntegerDelimitedField) annotation;
