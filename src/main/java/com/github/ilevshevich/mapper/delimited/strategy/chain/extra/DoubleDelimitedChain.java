...
import com.github.ilevshevich.mapper.delimited.annotation.extra.DoubleDelimitedField;
...
public class DoubleDelimitedChain<T, U extends DelimetedModel> implements DelimitedChain<T, U> {
    private DelimitedChain<T, U> next = null;

    public DoubleDelimitedChain(final DelimitedChain<T, U> next) {
        this.next = next;
    }

    public DoubleDelimitedChain() {
        this.next = null;
    }

    @Override
    public void execute(final Annotation annotation, final Field field, final List<T> data, final U tuple) throws Exception {
        if (annotation instanceof DoubleDelimitedField) {
            final DoubleDelimitedField fieldAnnotation = (DoubleDelimitedField) annotation;
