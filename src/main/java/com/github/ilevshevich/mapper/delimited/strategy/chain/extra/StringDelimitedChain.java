...
import com.github.ilevshevich.mapper.delimited.annotation.extra.StringDelimitedField;
...
public class StringDelimitedChain<T, U extends DelimetedModel> implements DelimitedChain<T, U> {
    private DelimitedChain<T, U> next = null;

    public StringDelimitedChain(final DelimitedChain<T, U> next) {
        this.next = next;
    }

    public StringDelimitedChain() {
        this.next = null;
    }

    @Override
    public void execute(final Annotation annotation, final Field field, final List<T> data, final U tuple) throws Exception {
        if (annotation instanceof StringDelimitedField) {
            final StringDelimitedField fieldAnnotation = (StringDelimitedField) annotation;
